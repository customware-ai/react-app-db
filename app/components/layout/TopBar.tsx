            ...
            <>
              {breadcrumbs.map((crumb, index) => (
                <div key={crumb.href || crumb.label} className="flex items-center gap-2">
                  ...
                  {crumb.href ? (
                    <Link to={crumb.href} className="text-surface-600 hover:text-surface-900 font-medium transition-colors">
                      {crumb.label}
                    </Link>
                  ) : (
                    <span className="text-surface-900 font-semibold">{crumb.label}</span>
                  )}
                </div>
              ))}
            </>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              ...
              <kbd className="px-2 py-0.5 text-xs bg-white border border-surface-200 rounded text-surface-500">
                ⌘K
              </kbd>
            </button>
            ...
            {searchOpen && (
              <>
                ...
                <div className="absolute top-full right-0 mt-2 w-96 bg-white rounded-xl shadow-strong border border-surface-200 z-50 overflow-hidden animate-scale-in">
                  <div className="p-3 border-b border-surface-200">
                    <input
                      type="text"
                      placeholder="Search customers, invoices, orders..."
                      className="w-full px-4 py-2 bg-surface-50 border border-surface-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                      autoFocus
                    />
                  </div>
                  <div className="p-2 max-h-96 overflow-y-auto">
                    <div className="px-3 py-2 text-xs font-semibold text-surface-500 uppercase tracking-wide">
                      Recent Searches
                    </div>
                    <div className="text-sm text-surface-500 px-3 py-8 text-center">
                      No recent searches
                    </div>
                  </div>
                </div>
          ...
          <div className="relative">
            ...
            {notificationsOpen && (
              <>
                ...
                <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-xl shadow-strong border border-surface-200 z-50 overflow-hidden animate-scale-in">
              ...
              <>
                ...
                <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-strong border border-surface-200 z-50 overflow-hidden animate-scale-in">
                  ...
                  <div className="p-2">
                    <Link
                      to="/settings"
                      className="flex items-center gap-3 px-3 py-2 text-sm text-surface-700 hover:bg-surface-50 rounded-lg transition-colors"
                    >
