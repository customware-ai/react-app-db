...
export async function createUser(name: string, email: string): Promise<SqlValue[] | null> {
...
export async function getUsers(): Promise<SqlValue[][]> {
  const { db } = await getDatabase();
  const result: QueryExecResult[] = db.exec('SELECT * FROM users ORDER BY id DESC');
...
export async function updateUser(id: number, name: string, email: string): Promise<SqlValue[] | null> {
  const { db } = await getDatabase();
  try {
...
export async function getAllTasks(): Promise<Record<number, SqlValue[][]>> {
  const { db } = await getDatabase();
  const result: QueryExecResult[] = db.exec('SELECT * FROM tasks ORDER BY user_id, id DESC');
